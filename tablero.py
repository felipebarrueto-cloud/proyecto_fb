...
def procesar_habilidades_carta(carta, marea_ya_cambio):
    ...
    # Sumar Æmbar impreso en la carta
...
def mostrar_tablero():
    ...
    st.markdown("""
        <style>
            div.stButton > button { background-color: #1a1c23 !important; color: #ffffff !important; border: 1px solid #333 !important; border-radius: 8px !important; height: 2.8em !important; }
            ...
            [data-testid="stHorizontalBlock"]:has(button[key*="btn_manual"]) { display: flex !important; flex-direction: row !important; flex-wrap: nowrap !important; gap: 10px !important; }
            [data-testid="stHorizontalBlock"]:has(button[key*="btn_manual"]) div[data-testid="column"] { width: 50% !important; flex: 1 1 50% !important; min-width: 50% !important; }
        </style>
    """, unsafe_allow_html=True)

    # --- 1. LÓGICA DE REVELADO CON GENERACIÓN DE RECURSOS ---
    if st.button("🎴 REVELAR SIGUIENTE CARTA", use_container_width=True):
        ...
        # A. GENERACIÓN POR RECOLECTORES (En mesa antes de avanzar)
        recursos_recolectores = sum(c['ambar_generado'] for c in st.session_state.mesa 
                                   if c.get('no_hace_danio') and c.get('ambar_generado'))
        if recursos_recolectores > 0:
            st.session_state.recursos_jefe += recursos_recolectores
            st.toast(f"✨ Recolectores: +{recursos_recolectores} Æ")
        ...
        # C. MOVER CARTA ANTERIOR A LA MESA
        ...
        # D. REVELAR NUEVA Y REGLA "SIN PRESA"
        if st.session_state.mazo:
    ...
    # --- 2. TABLA RESUMEN ---
    poder_total = sum(c.get('defensa', 0) for c in st.session_state.mesa 
    ...
    # Botones manuales (Gris y lado a lado)
